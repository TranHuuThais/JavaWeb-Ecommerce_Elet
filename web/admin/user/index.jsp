                                    ...
                                    <table class="table">
                                        ...
                                        <tbody>
                                            <c:forEach items="${userList}" var="user" varStatus="status">
                                                <tr>
                                                    <th scope="row">${(currentPage - 1) * pageSize + status.index + 1}</th> <!-- Correct STT Calculation -->
                                                    <td>${user.email}</td>
                                                    <td>${user.role}</td>
                                                    <td>
                                            ...
                                            <nav aria-label="Page navigation">
                                                <ul class="pagination justify-content-center">
                                                    <c:if test="${currentPage > 1}">
                                                        <li class="page-item">
                                                            <a href="IndexUserServlet?page=${currentPage - 1}" class="page-link" aria-label="Previous">
                                                    ...
                                                    <c:forEach var="i" begin="1" end="${totalPages}">
                                                        <li class="page-item ${currentPage == i ? 'active' : ''}">
                                                            <a href="IndexUserServlet?page=${i}" class="page-link">${i}</a>
                                                        </li>
                                                    </c:forEach>
                                                    <c:if test="${currentPage < totalPages}">
                                                        <li class="page-item">
                                                            <a href="IndexUserServlet?page=${currentPage + 1}" class="page-link" aria-label="Next">
